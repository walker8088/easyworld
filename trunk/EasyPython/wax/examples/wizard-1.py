...
from wax import *
from wax.tools.wizard import Wizard, WizardPage
import wx


lots_of_text = """
lorem ipsum dolerem amat. dolores amanda est? vero amicum habet, illum
nigrum grassum. si, vero faeces est.
lorem ipsum dolerem amat. dolores amanda est? vero amicum habet, illum
nigrum grassum. si, vero faeces est.
lorem ipsum dolerem amat. dolores amanda est? vero amicum habet, illum
nigrum grassum. si, vero faeces est.
...
class Page1(WizardPage):
    ...
    def Body(self):
        label1 = Label(self, text=lots_of_text)
        self.AddComponent(label1)
        self.skipbox = CheckBox(self, text="Check this box to skip the next page:")
        self.AddComponent(self.skipbox)
        self.Pack()

    def OnNextPage(self, event=None):
        if self.skipbox.Get3StateValue() == 'checked':
            self.SetTarget(2)
        else:
            self.SetTarget(1)
        WizardPage.ChangePage(self)

class Page2(WizardPage):
        
    def Body(self):
        z = TextBox(self, text=lots_of_text)
        self.AddComponent(z)
        z.Enable()
        self.Pack()
...
class Page3(WizardPage):
    ...
    def Body(self):
        z = TextBox(self, text='<enter some text>')
        self.AddComponent(z, border=10)
        z.Enable()
        self.Pack()

    def OnPreviousPage(self, event=None):
        if self.prev.prev.skipbox.Get3StateValue() == 'checked':
            self.SetTarget(0, enableback=False)
        else:
            self.SetTarget(1)
        WizardPage.ChangePage(self)
...
#
# image for Wizard, taken from wxPython demo
...
class MainFrame(Frame):
    def Body(self):
        b1 = Button(self, "Run wizard with image", event=self.RunWizard1)
        b2 = Button(self, "Run wizard without image", event=self.RunWizard2)
        self.AddComponent(b1, border=20)
        self.AddComponent(b2, border=20)
        self.Pack()

    def RunWizard1(self, event):
        wizard = MyWizard(self, 'Just some wizard', getWizTestBitmap())
        wizard.Run()

    def RunWizard2(self, event):
        wizard = MyWizard(self, 'Just some wizard')
        wizard.Run()
...
if __name__ == "__main__":
    app = Application(MainFrame, direction='v', title='wizard-1.py')
    app.Run()
