...
RED, BLACK, UNDEFINED = range( 3 )

def Opponent(colour):
    return not colour

PIECE_ABBREVIATIONS = "prncbak."
PAWN, ROOK, KNIGHT, CANNON, BISHOP, ADVISOR, KING, BLANK = PIECE_ABBREVIATIONS

ONGOING, MATE, STALEMATE = range(3)

PIECE_NAMES = { 'p':'Pawn', 'r':'Rook', 'n':'Knight', 'c':'Cannon', 'b':'Bishop', 'k':'King','a':'Advisor', '.':'Blank' }

INITIAL_BOARD = 'rnbakabnr\n' \
                '.........\n' \
                '.c.....c.\n' \
                'p.p.p.p.p\n' \
                '.........\n' \
...
def MakePiece(char=None, type=BLANK, colour=RED):
    ...
    if type == PAWN:
        ...
        return Cannon(colour)

    elif type == BISHOP:
        return Bishop(colour)

    elif type == ADVISOR:
        return Advisor(colour)
...
class Piece(object):
    ...
    def fen_letter(self):
        ...
        """Return the letter abbreviation for this piece, upper case for RED."""
...
class Knight(Piece):
    ...
    def can_potentialy_attack(self, board, coord):
        ...
        for m in Knight.MOVES:
            ...
            if hop in board and board[hop].position == self.position:
                return True
...
class Cannon(Piece):
    ...
    def __init__(self, colour, position=None):
        ...
        Piece.__init__(self, CANNON, colour, position=position)
...
class Advisor(Piece):
    ...
    def __init__(self, colour, position=None):
        ...
        Piece.__init__(self, ADVISOR, colour, position=position)
...
class King(Piece):
    ...
    def can_potentialy_attack(self, board, coord):
        ...
        for direction in (LEFT, RIGHT, UP, DOWN):
            ...
            if self.position == coord+direction:
                return True
...
class Board( object ):
    ...
    def __init__(self, cols=9, rows=10, blank=True):
    ...
    def get_turn(self):
        ...
        return (BLACK, RED)[self.move_count & 1]
    ...
    def set_blank(self, cols=9, rows=10):

        self.cols = cols
        self.rows = rows
        
        self.squares = tuple( [None for _ in xrange(self.cols)] for _ in xrange(self.rows) )
    ...
    def board_visual(self):
        ...
        for row in reversed(range(self.rows)):
            ...
            bv.append( ("     %i | "%(row)) + " ".join( [str(self.square(Coord(col,row)).fen_letter()) for col in range(self.cols)] ) + " |")
    ...
    def full_move_count(self):
        ...
        """Returns the full move number. A full move consists of REDs move and blacks response."""
    ...
    def check_threefold_repetition(self, moves, initial_board):
        ...
        for move in moves:
            ...
            fen = board.fen(simple=True)
            board_fens[fen] = board_fens.get(fen, 0) + 1

        if board_fens.get(fen, 0) >= 3:
            return True
...
class Game(object):
    ...
    def parse_movetext(self):
        ...
        for token in self.movetext.replace('.',' ').split():
